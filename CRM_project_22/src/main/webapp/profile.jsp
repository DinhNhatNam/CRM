							...
							<div class="user-bg">
								...
								<div class="overlay-box">
									<div class="user-content">
										...
										<h4 class="text-white">${listDetail.get(0).getId_NguoiDung().getFullname()}</h4>
										<h5 class="text-white">${listDetail.get(0).getId_NguoiDung().getEmail()}</h5>
									</div>
								</div>
							</div>
						...
						<div class="white-box">
							<div class="table-responsive">
								<table class="table" id="example">
									...
									<tbody>
									<%! int x = 0;%>
										<c:forEach items="${listDetail}" var="listJob">
										<% x=x+1; %>
											<tr>
												<td><%=x %></td>
												<td>${listJob.getId_CongViec().getName()}</td>
												<td>${listDuAn.get(0).getName()}</td>
												<td>${listJob.getId_CongViec().getNgaybatdau()}</td>
												<td>${listJob.getId_CongViec().getNgayketthuc()}</td>
												<td><c:if
														test="${listJob.getId_CongViec().getTrangthai().getId() == 1}">Chưa thực hiện</c:if>
													<c:if test="${listJob.getId_CongViec().getTrangthai().getId() == 2}">Đang thực hiện</c:if>
													<c:if test="${listJob.getId_CongViec().getTrangthai().getId() == 3}">Đã hoàn thành</c:if>
												</td>
												<td><a href="profile-edit" class="btn btn-sm btn-primary">Cập nhật</a></td>
											</tr>
										</c:forEach>
									</tbody>
								</table>
